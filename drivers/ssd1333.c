...
static inline int freq_to_nrffreq(int f) {
  if(f <= 125000) {
...
int ssd1333_init(const struct device *dev) {
  ...
  nrf_spim_frequency_set(NRF_SPIM3, freq_to_nrffreq(config->clock_frequency));
...
static int ssd1333_set_brightness(const struct device *dev, const uint8_t brightness_pct) {
  if(brightness_pct > 0 && brightness_pct <= 100) {
    const uint8_t v = 150 + (uint8_t)((brightness_pct/100) * (255 - 150));

    BYTE c[] = {v};
    printk("[SSD1333] SET BRIGHTNESS at %i (%i%%)\n", v, brightness_pct);
    return ssd1333_write_bus(dev, SSD1333_CMD_MASTERCURRENT, c, sizeof(c));
  }
...
static int ssd1333_set_contrast(const struct device *dev, const uint8_t contrast) {
  BYTE c[3] = {contrast};
  return ssd1333_write_bus(dev, SSD1333_CMD_CONTRAST_COLOR, SSD1333_DATA_CONTRAST_DEF, sizeof(c));
}
...
#define SSD1333_DEFINE(inst)                                                \
  static const struct ssd1333_config ssd1333_config_##inst = {              \
    ...
    .clock_frequency = DT_INST_PROP(inst, spi_max_frequency),               \
